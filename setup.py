...
def check_for_command_in_shellrc():
    ...
    if (
        INITIAL_SHELLRC_STRING not in shellrc_file
        and INITIATED_SHELLRC_STRING not in shellrc_file
    ):
        shellrc_file += INITIAL_SHELLRC_STRING
        if any(
            1 for x in shellrc_file.split("\n") \
            if x.startswith(sign_of_prjman_alias)
        ):
            shellrc_file_lines = shellrc_file.split("\n")
            change_items_index(
                shellrc_file_lines,
                [x for x in shellrc_file_lines \
                    if x.startswith(sign_of_prjman_alias)][0],
                len(shellrc_file_lines),
            )
